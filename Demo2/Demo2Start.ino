...
int motorSpeed = 80; //PWM signal out of 255
const int period = 20;  
...
//digitalWrite(m1dir, LOW);
 ...
 Serial.println(countsB);
  if (doneSearching && (!hasEncompassed)) //send 1 if aruco detected at 0 angle
  {
    //move camera distance minus a foot forward
    rhoCalcPI(0.3096); //subject to change
    if(oneFootAway) //send 2
    {
      if (!hasEncompassed)
      {
         encompass();
      ...
      //encompass(); //This function will make the robot encompass the beacon in a square
    }
  } else if(!hasEncompassed)
    {
      //rotating to find beacon
      phiCalcPI(2*PI); //subject to change
    }
...
void rotateToPhi(double phiSetPosition){
  Serial.print("ROTATION FUNCTION STARTED");
...
void encompass(void)
{
  ...
  rotateToPhi(PI/6);
  goForwards(.3096*2.25);
  rotateToPhi(-PI/2.5);
  goForwards(.3096*1.6);
  rotateToPhi(-PI/2);
  goForwards(.3096*1.85);
  rotateToPhi(-PI/2.1);
  goForwards(.3096*1.5);
